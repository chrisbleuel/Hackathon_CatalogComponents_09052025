...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div className="min-w-[280px] sm:min-w-[300px] max-w-[635px] self-stretch flex-1 shrink basis-[0%] bg-white pb-4 px-3 sm:px-6">
      <div className="flex w-full items-center gap-2 pr-2 pt-6 pb-2">
        <img
          ...
          className="aspect-[1] object-contain w-7 sm:w-8 self-stretch my-auto cursor-pointer"
          alt="Wishlist"
        />
      </div>
      ...
      <div className="w-full pb-2">
        <div className="flex flex-col relative min-h-[260px] sm:min-h-[342px] w-full overflow-hidden">
          <img
            ...
            alt={`Product ${model}`}
          />
          <div className="relative rotate-[0.27166683933431357rad] flex min-h-[260px] sm:min-h-[342px] w-full" />
        </div>
        ...
        <div className="w-full mt-2 px-6 sm:px-16">
          <div className="flex w-full items-stretch gap-[5px] justify-center">
            {images.map((_, index) => (
              <div
                key={index}
                className={`flex w-[80px] sm:w-[106px] shrink-0 h-1 ${
      ...
      <div className="flex w-full gap-4 whitespace-nowrap flex-wrap">
        ...
        {sizes.length > 0 && (
          <div className="flex gap-2 text-xl text-white font-bold text-center tracking-[0.1px] justify-center flex-wrap">
            {sizes.map((size) => (
              <div key={size} className="flex min-h-[19px] gap-1.5 leading-[1.4] w-[19px] mb-1">
        ...
        <div className="flex min-w-[240px] sm:min-w-60 w-full gap-2 flex-1 shrink basis-[0%] pr-2 py-6">
          <div className="flex gap-4 overflow-hidden">
            {colors.map((color, index) => (
              <div key={index} className="w-10 sm:w-12 flex-shrink-0" onClick={() => setSelectedColor(index)}>
                <div className={`justify-center items-center border-b-[color:var(--Global-Border-subtle,#C2CDD6)] flex min-h-11 w-full gap-2 pb-1 px-2 sm:px-3 border-b-2 border-solid ${
